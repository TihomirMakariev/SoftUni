...
public class WrongMeasurements {
    public static void main(String[] args) {
        ...
        List<int[]> updateValues = new ArrayList<>();

        for (int row = 0; row < matrix.length; row++) {
            for (int col = 0; col < matrix[row].length; col++) {
                if (matrix[row][col] == wrongValue){
                    updateValues.add(new int[] {row, col, getClosestItemsSum(row, col, matrix, wrongValue)});
                }
            }
        }

        for (int[] updateValue : updateValues) {
            matrix[updateValue[0]][updateValue[1]] = updateValue[2];
        }
